
<div class="sidebar border border-right col-md-3 col-lg-2 p-0 bg-body-tertiary" style="height:190%;">
      <div class="offcanvas-md offcanvas-end bg-body-tertiary" tabindex="-1" id="sidebarMenu" aria-labelledby="sidebarMenuLabel">
        <div class="offcanvas-header">
          <h5 class="offcanvas-title" id="sidebarMenuLabel">Company name</h5>
          <button type="button" class="btn-close" data-bs-dismiss="offcanvas" data-bs-target="#sidebarMenu" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body d-md-flex flex-column p-0 pt-lg-3 overflow-y-auto">
          <ul class="nav flex-column">
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2 active" aria-current="page" href="index.php">
                <svg class="bi"><use xlink:href="#house-fill"/></svg>
                Dashboard
              </a>
            </li>
            <h6 class="sidebar-heading d-flex justify-content-between align-items-center px-3 mt-4 mb-1 text-body-secondary text-uppercase">
          <span>Product</span>
          <a class="link-secondary" href="#" aria-label="Add a new Activity">
            <svg class="bi"><use xlink:href="#plus-circle"/></svg>
          </a>
        </h6>

        <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="viewcategory.php">
                <svg class="bi"><use xlink:href="#people"/></svg>
                Product Categories
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="addproduct.php">
                <svg class="bi"><use xlink:href="#file-earmark"/></svg>
                Add NewProduct
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="view_products.php">
                <svg class="bi"><use xlink:href="#file-earmark"/></svg>
                View Products
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="add_price.php">
                <svg class="bi"><use xlink:href="#cart"/></svg>
               Add Price
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="add_stockin.php">
                <svg class="bi"><use xlink:href="#cart"/></svg>
               Purchase Product
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="display_stockin.php">
                <svg class="bi"><use xlink:href="#cart"/></svg>
               View Purchased products
              </a>
            </li>            
           
            <h6 class="sidebar-heading d-flex justify-content-between align-items-center px-3 mt-4 mb-1 text-body-secondary text-uppercase">
          <span>supliers</span>
          <a class="link-secondary" href="#" aria-label="Add a new Activity">
            <svg class="bi"><use xlink:href="#plus-circle"/></svg>
          </a>
        </h6>
        <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="all_suplier.php">
                <svg class="bi"><use xlink:href="#file-earmark"/></svg>
                Manage supliers
              </a>
            </li>



        <h6 class="sidebar-heading d-flex justify-content-between align-items-center px-3 mt-4 mb-1 text-body-secondary text-uppercase">
          <span>Werahouse</span>
          <a class="link-secondary" href="#" aria-label="Add a new Activity">
            <svg class="bi"><use xlink:href="#plus-circle"/></svg>
          </a>
        </h6>
        <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="all_warehouse.php">
                <svg class="bi"><use xlink:href="#file-earmark"/></svg>
                Manage Werahouse
              </a>
            </li>
          
          </ul>

          <br><br>

          <hr class="my-3">
          <br><br>
          <ul class="nav flex-column mb-auto">
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="#">
                <svg class="bi"><use xlink:href="#gear-wide-connected"/></svg>
                Settings
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="../../authentication/logout.php">
                <svg class="bi"><use xlink:href="#door-closed"/></svg>
                Sign out
              </a>
            </li>
          </ul>
        </div>
      </div>

    </div>